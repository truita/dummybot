...
class Flags(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.db_dir = os.path.join(workingDirectory, "db")
        if not os.path.isdir(self.db_dir):
            os.mkdir(self.db_dir, 664)
        self.reset_flags.start()

    def _get_conn(self, guild_id):
        return sqlite3.connect(
            os.path.join(self.db_dir, f"{guild_id}.sqlite3"), isolation_level=None
        )
    ...
    def is_flag_ready(self, flagtype, guild, maker): # 0 = Flag is unavalaible, 1 = Flag is avalaible, *FlagType* = A user has already taken *FlagType* 
        if guild in self.readyflags:
            ...
            setattr(self.readyflags[guild], f"{flagtype}_maker", False)
            return 1
    ...
    async def pole(self, ctx):
        ...
        if flagstatus == 1:
            conn = self._get_conn(ctx.guild.id)
            conn.execute("INSERT INTO FLAGS(ID,POINTS,POLE) VALUES(?,4,1) ON CONFLICT(ID) DO UPDATE SET POINTS = POINTS + 4, POLE = POLE + 1", (ctx.author.id,))
    ...
    async def subpole(self, ctx):
        ...
        if flagstatus == 1:
            conn = self._get_conn(ctx.guild.id)
            conn.execute("INSERT INTO FLAGS(ID,POINTS,SUBPOLE) VALUES(?,2,1) ON CONFLICT(ID) DO UPDATE SET POINTS = POINTS + 2, SUBPOLE = SUBPOLE + 1", (ctx.author.id,))
    ...
    async def fail(self, ctx):
        ...
        if flagstatus == 1:
            conn = self._get_conn(ctx.guild.id)
            conn.execute("INSERT INTO FLAGS(ID,POINTS,FAIL) VALUES(?,1,1) ON CONFLICT(ID) DO UPDATE SET POINTS = POINTS + 1, FAIL = FAIL + 1", (ctx.author.id,))
    ...
    async def ranking(self, ctx):
        ...
        embed = discord.Embed(colour=discord.Colour.blue(), description=description)
